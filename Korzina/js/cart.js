document.addEventListener('DOMContentLoaded', () => {
		...
		return `
			<li class="cart-content__item">
				<article class="cart-content__product cart-product" data-id="${id}">
					...
					<button class="cart-product__delete" aria-label="Тауарды жою"></button>
				</article>
			</li>
		`;
	};
		...
		return `
			<li class="order-modal__item">
				<article class="order-modal__product order-product" data-id="${id}">
					...
					<button class="order-product__delete">Жою</button>
				</article>
			</li>
		`;
	};
	...
	document.querySelector('.order').addEventListener('submit', (e) => {
		...
		xhr.open('POST', 'http://127.0.0.1:5000/submit_order', true);
